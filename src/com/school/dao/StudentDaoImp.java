    ...
    public void insertStudentData(Student student){
     Connection conn = openConnection();
      ...
      String sql = "insert into students (STUD_ID,LEVEL_ID,`STUD_FIRST_NAME`,`STUD_MID_NAME`,`LAST_NAME`,`ADDRESS`,`GENDER`,`DATE_OF_BIRTH`,`PHONE`,PARENT_ID)"
                + "values (" +student.getStudentId() + ",'" + student.getStudentLevel() + "','" +student.getFirstName() + "','" +student.getMidName()  + "','" +student.getLastName() + "','" + student.getAddress() + "','" + student.getGender()+ "','" + student.getDateOfBirth() + "','" +student.getPhone() + "','" +student.getParent().getParentId()+  "')";
...
@Override
      public void updateStudentDtata(Student student,int studentId){
        String sql="update students set STUD_ID= "+student.getStudentId()+", STUD_LEVEL" + student.getStudentLevel()+ ",STUD_FIRST_NAME='"+student.getFirstName()+"', STUD_MID_NAME='"+student.getMidName()+"',LAST_NAME='"+ student.getLastName()+"',ADDRESS='"+student.getAddress()+"', GENDER='"+student.getGender()+"',DATE_OF_BIRTH='"+student.getDateOfBirth()+"',PHONE='"+student.getPhone()+"',PARENT_ID="+student.getParent().getParentId()+" where STUD_ID="+studentId;
        Connection conn=openConnection();
          updateQuery(sql, conn);
          closeConnection(conn);
      }
